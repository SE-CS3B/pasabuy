<template>
 <div>
  <Navbar />
    ...
    <div class="shadow-sm bg-white w-1/2 h-32 mb-3">
      <div class="right-0">
        <p class="float-right mr-16 pt-20 text-xl font-nunito font-bold">
          ACCOUNT SETTINGS
        </p>
      </div>
    </div>
    <div
      @click="isOpen = false"
      class="w-full flex justify-center self-center font-nunito mb-11"
    >
      <div
        class="w-5/12 flex flex-col justify-center self-center gap-y-5 vs:space-y-5"
      >
        <div
          id="personal_info"
          class="ring-1 ring-gray-300 bg-white p-5 rounded-2xl shadow-sm"
        >
          <div class="flex justify-between">
            ...
            <div>
              <label
                for="sub"
                id="edt1"
                @click="(hidden = !hidden), Edit('edt1'), (show = !show)"
          ...
          <div
            id="personal_info"
            class="text-sm w-1/2 gap-x-10 pt-8 vs:text-sm vs:w-full"
          >
            <form class="space-y-8">
              ...
              <span class="font-raleways font-bold grid grid-cols-2">
                ...
                <span>
                  ...
                  <input
                    @keyup="personal.birdate = $event.target.value"
                    v-if="hidden"
                    type="date"
                    id="start"
                    name="trip-start"
                    min="1990-01-01"
                    v-model="personal.birdate"
                  />
                </span>
              </span>
              <span class="font-raleways font-bold grid grid-cols-2">
          ...
          <div class="flex justify-between">
            ...
            <div>
              <label
                for="sub"
                id="edt2"
                @click="(edit2 = !edit2), Edit('edt2'), (show = !show)"
                class="text-blue-800 w-min font-bold text-sm cursor-pointer"
                >Edit</label
              >
            </div>
          </div>
          <div class="text-sm w-full gap-x-10 space-y-8 pt-8 vs:w-full">
            <span class="font-raleways font-bold grid grid-cols-3">
        ...
        <div class="ring-1 ring-gray-300 bg-white p-5 rounded-2xl shadow-sm">
          <div class="flex justify-between">
            ...
            <div>
              <p
                class="hover:text-gray-500 cursor-pointer tracking-wider text-blue-800 text-xs rounded-2xl h-min w-18 self-center p-1 font-nunito font-bold"
              >
                Edit
              </p>
            </div>
